...
class RouteGeometry(
    ...
    val allowedVertexSpeeds: DoubleArray
    ...
    init {
        ...
        for (i in n - 2 downTo 1) {
            val turnCap = turnCapAt(i)
            val brakingCap = sqrt(
                allowedVertexSpeeds[i + 1] * allowedVertexSpeeds[i + 1] +
                    2 * decelerationMps2 * (cumulative[i + 1] - cumulative[i]),
            )
            allowedVertexSpeeds[i] = min(turnCap, brakingCap)
        }
    ...
    companion object {
        ...
        const val DEFAULT_SPEED = 10.0
        const val MIN_TURN_SPEED = 2.0
