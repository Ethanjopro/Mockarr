...
- **Docs:** `docs/release/play-launch.md` (checkbox runbook, personal-account path, LLC path as
  appendix, tester brief), `docs/private/play-store-recommendations.md` (git-ignored since
  2026-09-08; the plain-text honest-notes doc he asked for: account choice, LLC triggers + Texas
  steps, Pro model, backend options table with quotas/terms as of today, what gets mock-location
  apps rejected, what I'm unsure about),
...
- **Not app work, recorded for the handoff:** Play Store launch tabled (walkthrough given; a new
  ...
  at 10800 twips like the other headers) + `docs/resume-mockarr-entry.txt`. Resume files are
  git-ignored (`docs/*.docx`, `docs/resume*`) — never commit them.
...
- **Personal-information sweep (same day, after Ethan asked):** the Play/LLC memo moved to
  `docs/private/play-store-recommendations.md` (git-ignored); a home-area place name and the
  state-specific LLC steps in `play-launch.md` were removed; `.gitignore` now covers
  `docs/private/`, `docs/portfolio/`, `docs/*.docx`, `docs/resume*`. **Scope, per Ethan:** redact
  only credentials and home-address-level detail; keep file paths (including `/Users/ethanjones/…`
  and resume filenames) because sessions rely on them. The commit author email was the one item a
  tree edit could not remove (see the rewrite note below).
